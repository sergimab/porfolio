...
function isoTileSrcs(name: string): string[] {
  ...
  if (!cat) return [];
  return cat.files.map((f) => `/proyectos/iberdrola/ilustraciones/${cat.folder}/${encodeURIComponent(f)}`);
}
...
function flatTileSrcs(name: string): string[] {
  ...
  return cat.files.map(
    (f) => `/proyectos/iberdrola/ilustraciones/${cat.folder}/${f.split("/").map(encodeURIComponent).join("/")}`
  );
}

// Escenas planas para los 2 recuadros del hero del bloque plano.
const FLAT_SCENES = ["1.svg", "2.svg"].map((f) => `/proyectos/iberdrola/ilustraciones/flat-escenas/${f}`);
          ...
          {b.iso ? (
            <div className="ilu-hero ilu-hero-tech">
              <div className="ilu-frame iso-scene">
                <img src="/proyectos/iberdrola/ilustraciones/isometric-escenas/Group%2050179.svg" alt="" />
              </div>
              <div className="ilu-frame iso-scene">
                <img src="/proyectos/iberdrola/ilustraciones/isometric-escenas/4.svg" alt="" />
              </div>
