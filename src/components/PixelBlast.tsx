...
// PixelBlast (React Bits) — port fiel a TypeScript.
// Inspirado en github.com/zavalit/bayer-dithering-webgl-demo
...
const SHAPE_MAP: Record<Variant, number> = { square: 0, circle: 1, triangle: 2, diamond: 3 };
...
const PixelBlast = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      // Pausa cuando el contenedor no está en pantalla (ahorro de GPU)
      let io: IntersectionObserver | null = null;
      if (autoPauseOffscreen) {
        io = new IntersectionObserver(
          (entries) => { visibilityRef.current.visible = entries[0]?.isIntersecting ?? true; },
          { threshold: 0 }
        );
      ...
      // Escuchamos en la ventana para que funcione aunque el canvas esté detrás
      ...
      threeRef.current = {
        ...
        resizeObserver: ro, intersectionObserver: io, raf, quad, timeOffset,
        composer, touch, liquidEffect, onPointerDown, onPointerMove,
      };
    } else {
      const t = threeRef.current;
